...
export default function ReportsPage() {
    ...
    const [activeTab, setActiveTab] = useState<"analytics" | "gst">("analytics");
    ...
    // ── GST Report Data ────────────────────────────────
    const gstSales = sales.filter((s) => {
        const saleMonth = new Date(s.sale_date).toISOString().slice(0, 7);
        return saleMonth === gstMonth;
    });

    const gstTaxableAmount = gstSales.reduce((s, r) => s + r.total_amount, 0);
    const gstAmount = (gstTaxableAmount * gstRate) / 100;
    ...
    gstSales.forEach((s) => {
        ...
        gstProductMap[name].amount += s.total_amount;
    });
    const gstProducts = Object.entries(gstProductMap)
        .sort((a, b) => b[1].amount - a[1].amount)
        .map(([name, d]) => ({ name, ...d }));

    function handleGSTDownload() {
        const rows = [
            ["BizPulse GST Report"],
            [`Month: ${new Date(gstMonth + "-01").toLocaleDateString("en-IN", { month: "long", year: "numeric" })}`],
            [`GST Rate: ${gstRate}%`],
            [],
            ["Product", "Units Sold", "Taxable Amount (₹)", `GST @ ${gstRate}% (₹)`, "Total (₹)"],
            ...gstProducts.map((p) => [
                p.name,
                p.units,
                p.amount.toFixed(2),
                ((p.amount * gstRate) / 100).toFixed(2),
                (p.amount + (p.amount * gstRate) / 100).toFixed(2),
            ]),
            [],
            ["TOTAL", "", gstTaxableAmount.toFixed(2), gstAmount.toFixed(2), gstTotalWithTax.toFixed(2)],
        ];
        ...
        a.href = url;
        a.download = `GST_Report_${gstMonth}.csv`;
        a.click();
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                    ...
                    <p className="text-gray-400 mt-1">
                        Track performance, profit, and GST for your business.
                    </p>
                </div>
                        ...
                        <>
                            ...
                            {topProducts.length > 0 && (
                                <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                                    ...
                                    {topProducts.map((p, i) => (
                                        <div key={p.name} className="grid grid-cols-5 gap-4 px-6 py-4 border-b border-gray-800/50 hover:bg-gray-800/30 transition items-center">
                                            <div className="col-span-2 flex items-center gap-3">
                                                <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" style={{ backgroundColor: COLORS[i] + "33", color: COLORS[i] }}>
                                                    {i + 1}
                                                </span>
                                                <span className="font-medium truncate">{p.name}</span>
                                            </div>
                                            <span className="text-gray-300">{p.units}</span>
                                            <span className="text-emerald-400 font-semibold">₹{p.revenue.toLocaleString("en-IN")}</span>
                            ...
                            <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                                ...
                                <div className="max-h-96 overflow-y-auto">
                                    {sales.map((sale) => {
                                        ...
                                        return (
                                            <div key={sale.id} className="grid grid-cols-6 gap-4 px-6 py-4 border-b border-gray-800/50 hover:bg-gray-800/30 transition items-center">
                                                <div className="col-span-2">
                                                    <p className="font-medium text-sm">{sale.products?.name ?? "Unknown"}</p>
                                                    <p className="text-gray-600 text-xs">
                                                        {new Date(sale.sale_date).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                                                    </p>
                                                </div>
                                                <span className="text-gray-400 text-sm">{sale.customers?.name ?? "Walk-in"}</span>
                                                <span className="text-gray-300 text-sm">{sale.quantity}</span>
                                                <div>
                                                    <p className="text-emerald-400 font-semibold text-sm">₹{sale.total_amount.toLocaleString("en-IN")}</p>
                                                    <p className={`text-xs ${profit >= 0 ? "text-teal-500" : "text-red-500"}`}>
                                                        {profit >= 0 ? "+" : ""}₹{profit.toLocaleString("en-IN")} profit
                                                    </p>
                                                </div>
                                                <span className={`text-xs px-2 py-1 rounded-full border w-fit ${sale.payment_status === "paid" ? "bg-emerald-900/30 border-emerald-700 text-emerald-300" : "bg-yellow-900/30 border-yellow-700 text-yellow-300"}`}>
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="flex items-center gap-4 flex-wrap">
                        <div className="flex flex-col gap-1">
                            <label className="text-xs text-gray-500 uppercase tracking-widest">Month</label>
                            <input
                                type="month"
                                value={gstMonth}
                                onChange={(e) => setGstMonth(e.target.value)}
                                className="bg-gray-900 border border-gray-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500"
                            />
                        </div>
                        <div className="flex flex-col gap-1">
                            <label className="text-xs text-gray-500 uppercase tracking-widest">GST Rate (%)</label>
                            <select
                                value={gstRate}
                        ...
                        <div className="flex flex-col gap-1 mt-5">
                            <button
                                ...
                                disabled={gstSales.length === 0}
                                className="bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white px-5 py-2.5 rounded-xl font-semibold text-sm transition"
                            >
                                ⬇️ Download CSV
                            </button>
                        </div>
                    ...
                    {gstSales.length === 0 ? (
                        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-16 flex flex-col items-center gap-3">
                            <span className="text-5xl">🧾</span>
                            <p className="text-gray-400 text-lg font-medium">No sales for this month</p>
                            <p className="text-gray-600 text-sm">Select a different month</p>
                        </div>
                    ) : (
                        <>
                            {/* GST Summary Cards */}
                                ...
                                {[
                                    {
                                        ...
                                        value: `₹${gstTaxableAmount.toLocaleString("en-IN")}`,
                                        icon: "💼",
                                        color: "from-blue-900/30 to-blue-800/10",
                                        border: "border-blue-800/50",
                                    },
                                    ...
                                    },
                                    {
                                        ...
                                        value: `₹${gstTotalWithTax.toLocaleString("en-IN")}`,
                                        icon: "💰",
                                        color: "from-emerald-900/30 to-emerald-800/10",
                                        border: "border-emerald-800/50",
                                    },
                            ...
                            <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                                <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
                                    <h2 className="text-lg font-semibold">🧾 Product-wise GST Breakdown</h2>
                                    <span className="text-gray-500 text-sm">
                                        {new Date(gstMonth + "-01").toLocaleDateString("en-IN", { month: "long", year: "numeric" })}
                                    </span>
                                </div>
                                ...
                                {gstProducts.map((p) => (
                                    ...
                                    >
                                        ...
                                        <span className="text-gray-400">{p.units}</span>
                                        <span className="text-emerald-400 font-semibold">
                                            ₹{p.amount.toLocaleString("en-IN")}
                                        </span>
                                        <span className="text-yellow-400 font-semibold">
                                            ₹{((p.amount * gstRate) / 100).toLocaleString("en-IN")}
                                        </span>
                                    </div>
                                ))}

                                {/* Total Row */}
                                <div className="grid grid-cols-5 gap-4 px-6 py-4 bg-gray-800/50 items-center">
                                    <span className="col-span-2 font-bold text-white">TOTAL</span>
                                    <span className="text-gray-400 font-bold">
                                        {gstProducts.reduce((s, p) => s + p.units, 0)}
                                    </span>
                                    <span className="text-emerald-400 font-bold">
                                        ₹{gstTaxableAmount.toLocaleString("en-IN")}
                                    </span>
                            ...
                            {/* Info note */}
                            <div className="bg-blue-900/20 border border-blue-800/50 rounded-2xl px-5 py-4 flex items-start gap-3">
                                <span className="text-xl mt-0.5">ℹ️</span>
                                <div>
                                    <p className="text-blue-300 font-medium text-sm">GST Disclaimer</p>
                                    <p className="text-blue-400/70 text-xs mt-1">
                                        This report is generated based on your recorded sales data. Please verify with a qualified CA before filing GST returns. BizPulse is not a substitute for professional tax advice.
                                    </p>
                                </div>
                            </div>
                        </>
